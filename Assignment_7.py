...
from Assignment_5 import posAndJac, scaling, boundNormal
from Assignment_6 import getXaArray


def integrand(data, j, fj, scale, w):
    """We create a function that represents the integrand 'N_a f_i det J',
...
def getExtForceVec(loads, basis, nodes, ien, ida, ncons):
    ...
    'basis' - the appropriate basis for the problem dimensionality
              (contains the basis function evals) indexed by
              [element region][int. pt. #][basis func. #]
                               [0 - value, 1, 2,... - dN/dxi_i]
    ...
    for i in range(len(loads[0])):  # for every element...
        ...
        for j in range(len(loads)):  # for every element region...
            ...
            for k in range(dims):  # for every degree of freedom...
                ...
                # for every item in the integral...
                for m in range(len(integral)):
                    # local number of the element basis function
                    a = int(m/dims)
                    # the degree of freedom for the element of 'integral'
                    jj = int(m % dims)
                    bigA = int(ien[i][a])  # global equation number

                    # if there is an available dof...
                    if ida[bigA*dims + jj] != 'n':
