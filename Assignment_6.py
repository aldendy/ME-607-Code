"""In this file, we develop a routine for calculating the internal force vector
for the Galerkin approximation of the weak form."""


import numpy as np
from Assignment_4 import getBasis
from Assignment_5 import posAndJac
from Assignment_6_Utils import getXaArray, getBandScale
from Assignment_6_Utils import getEulerStiff, getElemDefs
from Assignment_10 import getCauchy


def strainVec(numD: int, e: int, deform,
              ien: list[int, list[float, float, float]], Bmats: list):
    ...
    'Bmats' - set of 'B' matrices for all 'a' (of length = # basis funcs)
...
def stressVec(numD: int, strain: list, cCons='empty'):
...
def func(deform, basis, ien, e, i, xa, cCons='empty'):
    ...
    [c11, c12, c21, c22...] for 2D where c is indexed by [global node #]
                                                          [degree of freedom]
...
def GaussInt(func, deform, basis, e, ien, xa, cCons=0):
    """Here, we implement an updated version of Gaussian integration that
    relies on the 'basis' calculated in the 'getBasis' function. This
    eliminates the need to evaluate the functional as in the previous version.
...
def intForceVec(nodes, ien, ida, ncons, numD, numE, deform, cCons=0):
    ...
    calculates the internal energy integrals over the 2 or 3D domain describing
    the interior of all the elements (sigma).
    ...
    'deform' - a vector of the displacements (not positions) of all nodes with
    size [# nodes x 3]x[1]
    'cCons' - an array of the parameters needed to define the constituitive law
              that contains ['Young's Modulus', 'Poisson's Ratio']

    The output of this function is the internal force vector (global) of size =
    '[# nodes x 3]x[1]
    """
    basis = getBasis(numD)  # get the integration point and function data
    # initialize the internal force vector
    Fint = np.array((numD*len(nodes) - ncons)*[0.0])
    ...
    for i in range(numE):  # for every element...
        ...
        for j in range(len(basis[0][0])):  # for each basis function...
            for k in range(numD):  # for each dimension...
                ...
                # if there is an available dof...
                if ida[numD*int(ien[i][j]) + k] != 'n':
