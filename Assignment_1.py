...
"""In this program, we begin to implement some of the functions needed for a
multi-dimensional finite-element code.
"""


def nodeList(M: float, N: float, P: float, m: int, n: int = 0, p: int = 0):
    """This function takes parameters describing the essential characteristics
    of avtensor-product mesh and generates the corresponding 1D, 2D or 3D mesh
    of nodal positions.
    ...
    Dimensions other than 'x' default to zero dimensions.
    """
    nodes = []  # an array of size [num. of nodes]x[num. dimensions]
    stepi = 1.0*M/m

    if (n > 0) and (p == 0):
        # If we have a two-dimensional array of nodes...
        stepj = 1.0*N/n
    ...
    if (n > 0) and (p > 0):
        # If we have a three-dimensional array of nodes...
        stepj = 1.0*N/n
        stepk = 1.0*P/p
        ...
        for k in range(p+1):
            for j in range(n+1):
                for i in range(m+1):
                    node_number = k*(m+1)*(n+1) + j*(m+1) + i
                    nodes[node_number] = [i*stepi, j*stepj, k*stepk]

    if (n == 0) and (p > 0):
        # If it's 2D in x and z...
        stepk = 1.0*P/p
    ...
    if (n == 0) and (p == 0):
        # If we just need a 1D array of nodes...
        stepi = 1.0*M/m
...
def get_ien(m: int, n: int = 0, p: int = 0):
    """In this function, we assemble the connectivity array. The inputs to this
    function are the number of elements in each dimension of the tensor mesh.
    Only the number of elements in the first dimension is obligatory.
    ...
    if (n > 0) and (p == 0):
        # the connectivity array with two edges worth of nodes
        ien = np.zeros([m*n, size**2])
    ...
    if (n > 0) and (p > 0):
        # the connectivity array with three edges worth of nodes
        ien = np.zeros([m*n*p, size**3])
...
def getIDArray(cons: list[list]):
    """Constructing the stiffness matrix requires knowledge of the 'ID' array
    mapping the problem degrees of freedom onto the available degrees of
    freedom.
