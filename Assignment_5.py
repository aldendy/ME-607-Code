"""In this file, we implement the functions described in Lecture 4. These
functions rely on functions written in 'Assignment_4.txt'.
"""

import numpy
from Assignment_1 import get_ien, nodeList
from Assignment_4 import twoDBasis, threeDBasis


def posAndJac(pts, xa):
    """In this first function, we find the real 3D coordinate, x, and the
    Jacobian matrix for a given integration point with coordinates given in the
    parent domain. This is done for the 2D and 3D cases.
    ...
    'jac' - an array of all d(x_i)/d(xi_j) for all 'i (0, 1, 2)' and 'j
            (0 or 0,1 or 0,1,2)'
    """
    # because 'pts' may have differing numbers of basis funcs, go through loop
    x = [0, 0, 0]  # the physical coordinate initialized to the origin

    for i in range(len(pts)):  # for each basis function...
        # add the basis func times the correct component of 'xa'
        x[0] += xa[i][0]*pts[i][0]
        x[1] += xa[i][1]*pts[i][0]
        x[2] += xa[i][2]*pts[i][0]
...
def scaling(s, jac):
    """The second function we write takes the element region, 's', and the
    Jacobian and produces the appropriate integral scaling.
...
def realN(pts, a, jac):
    """Finally, we implement a function that performs a change of coordinates
    on the derivatives of N, transferring it from the parent domain to the
    physical domain.
    ...
    The function returns a vector, 'dNa/dxi', of all the 'xi' derivatives of
    a-th 'N' (real-dimensional derivatives)
    """
...
def routineTest():
    ...
    nodes3 = nodeList(esize[0], esize[1], esize[2],
                      edim3d[0], edim3d[1], edim3d[2])
...
def eLoadData(s):
    """The last function to implement is the element loop described in the
    assignment. The input is:
    's' - the element region (0 - interior, 1, 2 ... - faces or walls)
    """
    ...
    for i in range(eNum):  # for every element in the mesh...
        ...
        nodes = nodeList(esize[0], esize[1], esize[2],
                         edim[0], edim[1], edim[2])
        ...
        if edim[2] == 0:  # get basis depending on the dimension of problem
            basis = twoDBasis()
        else:
            basis = threeDBasis()

        # for every integration point in the region 's'...
        for j in range(len(basis[s])):
