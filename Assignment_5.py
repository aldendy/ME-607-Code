...
# Jacobian matrix for a givenintegration point with coordinates given
# in the parent domain. This is done for the 2D and 3D cases.
...
def posAndJac(pts, xa):
	...
	x[0] += xa[i][0]*pts[i][0]  # add the basis func times the 
	x[1] += xa[i][1]*pts[i][0]  # correct component of 'xa'
	x[2] += xa[i][2]*pts[i][0]
...
# The outputs are:
# 'norm' - the 3D normal vector for the boundary region
