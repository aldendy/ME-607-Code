"""In this file, we implement Gaussian quadrature rules over one, two and 
three dimensions.
...
's' - the part of the element over which to integrate (0 - interior, 1, 2, 3,
                                                       4 - sides)
"""
def twoDGauss(func, s):
	d = 1.0/3**0.5
	...
	for i in range(len(intpts[s])):
		area += intpts[s][i][1]*func(intpts[s][i][0][0], intpts[s][i][0][1])
...
The inputs are:
'func' - the integrand with inputs xi, eta, zeta and domain (-1, 1) x (-1, 1) x
         (-1, 1)
's'    - part of the element over which to integrate (0 for the interior, 1, 2,
                                                      3, 4, 5, 6 for the sides)
"""
def threeDGauss(func, s):
	d = 1.0/3**0.5
	# A table of integration points and weights where the indexes are
	# [interior (0) or one of four surfaces, integration point #] and 
	# the data stores is a [point coordinate in 'xi' and 'eta', weight]
	intpts = [[[[-d, -d, -d], 1], [[d, -d, -d], 1], [[-d, d, -d], 1], [[d, d, -d], 1],
		[[-d, -d, d], 1], [[d, -d, d], 1], [[-d, d, d], 1], [[d, d, d], 1]],
		[[[-1, -d, -d], 1], [[-1, d, -d], 1], [[-1, -d, d], 1], [[-1, d, d], 1]],
		[[[1, -d, -d], 1], [[1, -d, -d], 1], [[1, -d, -d], 1], [[1, -d, -d], 1]],
		[[[-d, -1, -d], 1], [[d, -1, -d], 1], [[-d, -1, d], 1], [[d, -1, d], 1]],
		[[[-d, 1, -d], 1], [[d, 1, -d], 1], [[-d, 1, d], 1], [[d, 1, d], 1]],
		[[[-d, -d, -1], 1], [[d, -d, -1], 1], [[-d, d, -1], 1], [[d, d, -1], 1]],
		[[[-d, -d, 1], 1], [[d, -d, 1], 1], [[-d, d, 1], 1], [[d, d, 1], 1]]]
	
	area = 0  # initialize the integral result
	for i in range(len(intpts[s])):
		area += intpts[s][i][1]*func(intpts[s][i][0][0], intpts[s][i][0][1], intpts[s][i][0][2])
...
The inputs are:
'n' - the number of dimensions (1, 2, or 3)
's' - part of the element over which to integrate (0 for the interior, 1, 2,
                                                   3, 4, 5, 6 for the sides)
'func' - the integrand with inputs xi, eta, zeta and domain (-1, 1) x (-1, 1) x
         (-1, 1)
"""
def GaussInt(n, s, func):
