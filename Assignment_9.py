"""Here, we implement routines necessary to solve more general problems.
...
from Assignment_6 import getXaArray, getBandScale
from Assignment_6_Utils import getElemDefs
from Assignment_10 import getCauchy


def position(node, axis, origin):
    """In order to properly select points, we must be able to compute the
    correct 'distance' to see if the node meets the selection criteria.

    The inputs are:
    'node'   - an individual, 3D coordinate for a node
    'axis'   - axis on which the node position is measured ('x', 'y', 'z', 'r')
    'origin' - origin from which the criteria will be measured
...
def nsel(direction, selType, loc, tol, nodes, nodeNums=0):
    ...
    This will give use the number of elements, the nodes for each element and
    the nodal locations. We then need to constrain them and get the 'id' array,
    number of constraints and the 'cons' array.
    ...
    'nodeNums' - 'list' of the numbers (0, 1, 2...) of currently selected nodes

    The return is:
    'nodeSet' - node numbers (0, 1, 2...) locating each member of 'nodeSet' in
    ...
    if selType == 'n':  # if we are making a new selection...
        ...
        for i in range(len(nodes)):  # for every node in the mesh...
            ...
            p = position(nodes[i], direction, [0, 0, 0])
            if (p > (loc - tol)) and (p < (loc + tol)):  # if in bounds...
                if i not in nodeNums:  # if node is not currently selected...
                    nodeSet.append(i)
                    nodeSet.sort()

    elif selType == 's':  # selecting a subset of the current set...
        # for every selected node in the mesh...
...
def constrain(nodes, selSet, ien, dof, d0, cons0='n'):
    """After getting the nodes, we now need to constrain all appropriate nodes.

    The inputs are:
    'nodes' - an array of the 3D locations of every node in the mesh
    'selSet' - selected node set (location of node in the 'nodes' array
                                  (0, 1...))
    ...
    """
    # dictionary maping the number of element nodes to
    dims = {2: 1, 4: 2, 8: 3}
    # the number of dimensions
    # maps the degree of freedom of interest to
    dofMap = {'x': 0, 'y': 1, 'z': 2}
...
def getMises(s):
    ...
    """
    if len(s) == 6:  # in 3D...
        a = (s[0][0] - s[1][0])**2 + (s[1][0] -
                                      s[2][0])**2 + (s[2][0] - s[0][0])**2
...
def getSigmaR(pt, s, p, axis):
    """This function returns cylindrical stress at a given point given the node
    location and stress vector (Voigt notation) at the point. It assumes that
    the problem has dimension of at least 2.

    The inputs are:
    'pt'   - the 3D coordinate of the point
    's'    - stress vector in the given number of problem dimensions (1-3)
    'p'    - direction of the stress ('t' - tangential, 'r' - radial)
    ...
    r = np.array(pt) - np.dot(np.array(pt), z) * \
        z  # point radial position from z
...
def get_stress_sol(deform, ien, nodes, stype, cCons=0):
    """An important function needed for plotting is a method for finding
    the stress at the nodes.

    The inputs are:
    'deform' - deformations for each degree of freedom of each node indexed by
    ...
    """
    stypeMap3 = {'sigma_x': 0, 'sigma_y': 1, 'sigma_z': 2, 'tau_xy': 5,
                 'tau_yz': 3, 'tau_zx': 4}  # maps types to numbers
...
def plotResults(deform, nodes, selSet, plotDir, dof, s='no'):
    """Another essential component to more advanced analyses is capability to
...
def contourPlot(deform, ien, nodes, stype, view, selset='none', cCons=0,
                s='no', size=None):
    ...
    'stype' - the type of stress ('sigma_x', 'sigma_y', 'sigma_z', 'tau_xy',
              'tau_yz', 'tau_zx', 'von Mises', 'sigma_r', 'sigma_t', 'd_abs',
              'd_x', 'd_y', 'd_z')
    'view' - picks one of the cardinal directions ('x', 'y', 'z') as viewpoint
    ...
    """
    titleMap = {'tau_xy': 'tau_{xy}', 'tau_yz': 'tau_{yz}',
                'tau_zx': 'tau_{zx}', 'd_abs': 'delta_{abs}', 'd_x': 'delta_x',
                'd_y': 'delta_y', 'd_z': 'delta_z'}
    if cCons != 0:
    ...
    # for plots of small nums...
    if span < 10**-12 and max(z, key=abs) < 2*span:
