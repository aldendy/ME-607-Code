...
"""This function takes parameters describing the essential characteristics of a
tensor-product mesh and generates the corresponding 1D, 2D or 3D mesh of nodal
positions.
...
def nodeList(M, N, P, m, n=0, p=0): 
    ...
    if (n > 0) and (p > 0):
        ...
        for k in range(p+1):
            for j in range(n+1):
                for i in range(m+1):
                    position = k*(m+1)*(n+1) + j*(m+1) + i
                    nodes[position] = [i*stepi, j*stepj, k*stepk]
...
function are the number of elements in each dimension of the tensor mesh. Only
the number of elements in the first dimension is obligatory.
