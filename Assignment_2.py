...
def load_and_cons(e: int, n: int, nd: int):
    ...
    'cons'  - a blank array of size [# dimensions]x[# nodes] containing 'n'
              when no constraint exists and a number when it is (initialized to
                                                                 all 'n')
    'loads' - an array of size [# element regions (s)]x[# elements] storing 'n'
              when no constraint exists, a number for a pressure and a vector
              for a traction or body force (initialized to all 'n')
    """
