...
class IntegralComponentTest(unittest.TestCase):
    """This class tests geometric functions in 'Assignment_5'. Important
    sub-tests in include parent domain to real translations, jacobians,
    ...
    def test_posAndJac2D(self):
        ...
        # a vector of all the calculated coordinates
        # [integration point #][dimension x, y, z]
        x = []
        # a vector of the jacobians for every integration point
        # [int. pt #][jacobian 3x2]
        jac = []
        ...
        for i in range(2):
            for j in range(2):
                # get real values for the real integration point coordinates
    ...
    def test_posAndJac3D(self):
        ...
        xa = [[1, 1, 0], [2, 1, 0], [1, 2, 0], [2, 2, 0], [1, 1, 1],
              [2, 1, 1], [1, 2, 1], [2, 2, 1]]
        # a vector of all the calculated coordinates [integration point #]
        # [dimension x, y, z]
        x = []
        # a vector of the jacobians for every integration point [int. pt #]
        # [jacobian 3x2]
        jac = []
        ...
        sets = [d, 1-d]
        x0 = []  # stores the true x-coordinates
        # stores the true jacobian
        jac0 = [[0.5, 0, 0], [0, 0.5, 0], [0, 0, 0.5]]
        for i in range(2):
            for j in range(2):
                for k in range(2):
                    # get real values for real integration point coordinates
