"""In this file, we implement the test cases used to verify the code being
developed for ME 607"""

import unittest
import numpy as np
from Assignment_6_Utils import getBandScale, numDims, getXaArray
from Assignment_6 import strainVec, stressVec, func, intForceVec
...
class MechanicsTest(unittest.TestCase):
    """This class implements some tests on the stress and strain computations
    in the code. It verifies basic mechanics capability in the code."""

    def setUp(self):  # define basic, needed parameters.
        """next, we test the 'strainVec' function. This test shows that all
        simple, one-dimensional strains are correctly calculated."""
        self.b1 = getBasis(1)
        self.numD = 1
        self.strain = 0.1
    ...
    def test_1DstrainCalcs(self):
        strain = strainVec(1, self.enum1, self.deform1, self.ien1, self.Bmats1)
    ...
    def test_2DstrainCalcs(self):
        ...
        for i in range(len(strain)):  # for every component...
            self.assertAlmostEqual(strain[i][0], correct[i])
    ...
    def test_2Dstress(self):
        ...
        for i in range(len(stress)):  # for each component...
            self.assertAlmostEqual(stress[i][0], correct[i], 5)
    ...
    def test_strainVecSimpleNormal(self):
        ...
        basis = getBasis(numD)
        # get the global coordinates of the element nodes
        xa = getXaArray(e, nodes, ien)

        # In this section, we test normal strain computations where 'epsilon =
        # 0.12' in each direction
    ...
    def test_strainVecShear(self):
        ...
        # In this section, we test strains 'epsilon_i,i+1' direction where i+1
        # can loop around to zero.
        for m in range(numD):  # for every direction...
            ...
            for i in range(len(nodes)):  # for every element node...
                for j in range(numD):  # for every dimension
                    if j == m:  # for the x-direction...
                        # add a strain
                        deform[numD*i + (j + 1) % numD] = nodes[i][j]*0.12
    ...
    # Here, we test the ability of the routine to correctly find the stress
...
class IntForceVectorFuncTest(unittest.TestCase):
    ...
    def test_funcOutput(self):
        ...
        for i in range(len(answer1)):  # for every component of 'answer'
            self.assertAlmostEqual(answer1[i][0]/correct1[i], 1)
...
class IntForceVecAssemblyTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cons, loads = load_and_cons(
            self.numE2, len(self.nodes2), 2)  # 2 dimensions
        cons[0][0] = 0
        cons[1][0] = 0
        cons[0][3] = 0
        cons[1][3] = 0
        ...
        cons, loads = load_and_cons(
            self.numE3, len(self.nodes3), 3)  # 2 dimensions
        cons[0][0] = 0
        cons[1][0] = 0
        cons[2][0] = 0
        cons[0][3] = 0
    ...
    def test_intForceVecOutput1D(self):
        Fint = intForceVec(self.nodes1, self.ien1, self.ida1, self.ncons1,
                           self.ndim1, self.numE1, self.deform1)
        correct = [2e10]
    ...
    def test_intForceVecOutput2D(self):
        Fint = intForceVec(self.nodes2, self.ien2, self.ida2, self.ncons2,
                           self.ndim2, self.numE2, self.deform2)
    ...
    def test_intForceVecOutput3D(self):
        Fint = intForceVec(self.nodes3, self.ien3, self.ida3, self.ncons3,
                           self.ndim3, self.numE3, self.deform3)
