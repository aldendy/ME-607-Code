...
class nselTests(unittest.TestCase):
    ...
    def test_AdditionalSelection(self):
        ...
        # intersect with new set
        s2 = nsel('y', 'a', 1.5, 0.01, self.nodes, s0)
    ...
    def test_SubsetSelection(self):
        ...
        # intersect with new set
        s2 = nsel('y', 's', 1.5, 0.01, self.nodes, s0)
    ...
    # Add an additional test for making a spherical selection about the origin
...
class plotDataTest(unittest.TestCase):
    """Now, test the ability of the plotResults function to corrrectly display
    2D data."""
...
class PressurizedCylinderTest(unittest.TestCase):
    """Another test involves solving problems with unaligned
    ...
    def test_accuracyPressCylinSol(self):
        ...
        for i in range(self.nr):  # for every radial element...
            ...
            u0 = a*b0  # exact solution at the first gauss quadrature point
            u1 = a*b1  # exact solution at the second gauss quadrature point
            us0 = 0.5*(1 + d)*dsol[i] + 0.5*(1 - d) * \
                dsol[i+1]  # solution deform
            us1 = 0.5*(1 - d)*dsol[i] + 0.5*(1 + d) * \
                dsol[i+1]  # solution deform
