...
complete finite-element problem. Multiple functions and the stiffness matrix
will be needed.
"""
...
def getEnergyDensity(D, Ba, Bb):
    ...
    'xa' - a vector of the 3D coordinates of the element nodes of size
                  [# element nodes]x[3]
    'a' - first elemental matrix subcoordinate (A in the notes) (0, 1, ...)
    'b' - second elemental matrix subcoordinate (B in the notes) (0, 1, ...)

    The outputs are:
    'k_ab' - a matrix of energy densities
    """
...
def gaussIntKMat(dims: int, xa, defE, cCons=0):
    ...
    'ke' - the element stiffness matrix of dimensions
           [(# dims)x(# element basis funcs.)] x
             [(# dims)x(# element basisfuncs.)]
    """
    ...
    basis = getBasis(dims)
    numA = round(2**dims)
    num_e = round(dims * numA)
    ya = np.array(xa) + np.array(defE)

    w = 1  # the gauss point integral weight (2 pts)
    ke = np.array([[0.0 for i in range(num_e)] for j in range(num_e)])
...
def getStiffMatrix(nodes, ien, ida, deform0, ncons, cCons=0):
    ...
    'ida'     - array mapping (Eqn. #) = ID[Eqn. # including restrained dof's]
    ...
    'kmat' - the global stiffness matrix of size
             [(# dims)x(# global basis funcs.)] x
               [(# dims)x(# global basis funcs.)]
    """
    dims = len(ida)/len(nodes)  # 'ida' has (# dims) as many
    totA = round(len(nodes)*dims - ncons)  # number of stiffness mat. equations
    ...
    for i in range(len(ien)):  # for every element...
        ...
        for j in range(len(ke)):  # for every row in 'ke'...
            for k in range(len(ke[0])):      # for every column in 'ke'...
                local_num = floor(j/dims)
                P = int(ien[i][local_num])  # the global node row number
                local_num = floor(k/dims)
                Q = int(ien[i][local_num])  # the global node column number
                pp = j % dims      # the dof num. for the row number
                qq = k % dims      # the dof num. for the column number

                p_location = round(P*dims + pp)
                q_location = round(Q*dims + qq)
                if (ida[p_location] != 'n') and (ida[q_location] != 'n'):
                    kmat[ida[p_location]][ida[q_location]] += ke[j][k]
    return kmat
...
def solver(numD, loads, nodes, ien, ida, ncons, cons, cCons=0):
    ...
    'loads' - an array containing either traction vectors, scalar pressures or
              chars indicating no load indexed by
